...
export const NewvelApi = {
  ...
  // 소셜 로그인
  logIn: () => springApi.get("/oauth2/authorization/kakao", { withCredentials: true }),
  // 로그인 api 이게 맞나요...??
  ...
  // 좋아요한 소설 표지 불러오기
  likesCovers: () => springApi.get("/covers/likes", { withCredentials: true }),

  // 구매한 소설 표지
  purchasedCovers: () => springApi.get("/covers/purchased-on", { withCredentials: true }),

  // episodes

  // 에피소드 구매목록 조회
  purchasedEpisodes: () => springApi.get("/episodes/purchased-on", { withCredentials: true }),

  // assets

  // 내가 가지고 있는 에셋 조회
  purchasedAssets: () => springApi.get("/assets/purchased-on", { withCredentials: true }),
